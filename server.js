...
(async function initDB() {
  ...
  // 帳號不存在才建立，已存在不覆蓋密碼
  const adminCheck = await db.execute(`SELECT id FROM admin_user WHERE username = ?`, ["admin"]);
  ...
  if (staffCheck.rows.length === 0) {
    ...
    await db.execute(`INSERT INTO admin_user(username, password, role) VALUES (?,?,?)`, ["staff01", hashStaff, "staff"]);
    console.log("✅已建立員工帳號：staff01 / 123456");
  }
})();
